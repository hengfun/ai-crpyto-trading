...
class DataHandler(object):
    ...
    def _query_cryptocompare(self,url: str, errorCheck: bool = True, api_key: str = None) -> Optional[Dict]:
        """
        ...
        """
        api_key_parameter = self._set_api_key_parameter(api_key)
        try:
    ...
    def _format_parameter(self,parameter: object) -> str:
        """
    ...
    def _format_timestamp(self,timestamp: Timestamp) -> int:
        """
    ...
    def _set_api_key_parameter(self,api_key: str = None) -> str:
        if api_key is None:
    ...
    def get_coin_list(self,format: bool = False) -> Union[Dict, List, None]:
        """
        ...
        """
        response = self._query_cryptocompare(_URL_COIN_LIST, False)
        if response:
    ...
    def get_price(self,coin: str, currency: str = CURRENCY, full: bool = False) -> Optional[Dict]:
        """
        ...
        """
        if full:
            return self._query_cryptocompare(
                _URL_PRICE_MULTI_FULL.format(
                    self._format_parameter(coin), self._format_parameter(currency))
            )
        if isinstance(coin, list):
            return self._query_cryptocompare(
                _URL_PRICE_MULTI.format(self._format_parameter(coin),
                                        self._format_parameter(currency))
            )
        return self._query_cryptocompare(
            _URL_PRICE.format(coin, self._format_parameter(currency))
        )


    def get_historical_price(self,coin: str, currency: str = CURRENCY, timestamp: Timestamp = time.time(),
        ...
        return self._query_cryptocompare(
            _URL_HIST_PRICE.format(coin,
                                ...
                                self._format_parameter(exchange))
        )


    def get_historical_price_day(self,coin: str, currency: str = CURRENCY, limit: int = LIMIT,
        ...
        response = self._query_cryptocompare(
            _URL_HIST_PRICE_DAY.format(coin, self._format_parameter(currency), limit, exchange, self._format_timestamp(toTs)))
        if response:
            return response['Data']
        return None


    def get_historical_price_hour(self,coin: str, currency: str = CURRENCY, limit: int = LIMIT,
                                exchange: str = 'CCCAGG', toTs: Timestamp = time.time()) -> Optional[Dict]:
        """
        ...
        """
        response = self._query_cryptocompare(
            _URL_HIST_PRICE_HOUR.format(coin, self._format_parameter(currency), limit, exchange, self._format_timestamp(toTs)))
        if response:
            return response['Data']
        return None


    def get_historical_price_minute(self,coin: str, currency: str = CURRENCY, limit: int = LIMIT,
                                    exchange: str = 'CCCAGG', toTs: Timestamp = time.time()) -> Optional[Dict]:
        """
        ...
        """
        response = self._query_cryptocompare(
            _URL_HIST_PRICE_MINUTE.format(coin, self._format_parameter(currency), limit, exchange, self._format_timestamp(toTs)))
        if response:
            return response['Data']
        return None


    def get_avg(self,coin: str, currency: str = CURRENCY, exchange: str = 'CCCAGG') -> Optional[Dict]:
        """
        ...
        :returns: dict of coin and currency price pairs
        """
        response = self._query_cryptocompare(_URL_AVG.format(
            coin, currency, self._format_parameter(exchange)))
        if response:
            return response['RAW']
        return None


    def get_exchanges(self,) -> Optional[Dict]:
        """
        Get the list of available exchanges.
        :returns: list of available exchanges
        """
        response = self._query_cryptocompare(_URL_EXCHANGES)
        if response:
            return response['Data']
        return None


    def get_pairs(self,exchange: str = None) -> Optional[Dict]:
        """
        ...
        """
        if exchange is None:
            response = self._query_cryptocompare(_URL_PAIRS.split('?')[0])

        else:
            response = self._query_cryptocompare(_URL_PAIRS.format(exchange))
        if response:
            return response['Data']
        return None

if __name__=="__main__":
    key = "f368b402-33e7-4cda-a8ab-470bb79681c0"
    dh = DataHandler()
    dh._set_api_key_parameter(key)
